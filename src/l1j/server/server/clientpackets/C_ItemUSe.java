...
import l1j.william.L1WilliamTeleportScroll; // DB道具傳送卷軸(符) by 丫傑
import l1j.kinlinlo.L1Blend; // 道具融合系統 by 狼人香
...
public class C_ItemUSe extends ClientBasePacket {
		...
		if (pc.getCurrentHp() > 0) {
			...
			if ((itemId == 40077)
				...
				Enchant.scrollOfEnchantWeaponIvoryTower(pc, l1iteminstance,
					...
					|| (itemId == 41431) || (itemId == 41432)) { // 武器屬性強化卷軸
				...
				} else if (l1iteminstance.getItem().getType() == 18) { // 道具傳送卷軸(符)類
				...
				} else if (l1iteminstance.getItem().getType() == 19) { // 道具融合系統類
					if (L1Blend.checkItemId(itemId) != 0) {
						L1Blend.getItemBlend(pc, l1iteminstance, itemId);
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。
					}
				} else if (l1iteminstance.getItem().getType() == 20) { // 家具類
				...
				// 治癒藥水、濃縮體力恢復劑、象牙塔治癒藥水
				...
				} else if ((itemId >= 41417) && (itemId <= 41421)) { // 日本「亞丁的夏天」活動道具
																		// - 刨冰
					...
					Potion.ThirdSpeed(pc, l1iteminstance, 600);
						...
						&& (itemId <= L1ItemId.SCROLL_FOR_ENCHANTING_BATTLE)) { // 150%神力藥水
																				// ~
																				// 強化戰鬥卷軸
					...
					if (l1iteminstance1.getItemId() >= 47053
						...
						if (Config.MAGIC_STONE_TYPE < rnd) {
							int newItem = l1iteminstance1.getItemId() + type; // 附魔石(近戰)
																				// ~
																				// 9階附魔石(近戰)
							...
							pc.sendPackets(new S_ServerMessage(1411,
									l1iteminstance1.getName())); // 對\f1%0附加魔法失敗。
						}
						pc.getInventory().removeItem(l1iteminstance1, 1); // 刪除
																			// -
																			// 附魔石
						pc.getInventory().removeItem(l1iteminstance, 1); // 刪除 -
																			// 附魔轉換卷軸
					} else {
						pc.sendPackets(new S_ServerMessage(79));
					}
					...
					if (Config.MAGIC_STONE_LEVEL < rnd
						...
						int newItem = l1iteminstance1.getItemId() + 1; // X 階附魔石
																		// ->
																		// X+1
																		// 階附魔石
						...
						pc.sendPackets(new S_ServerMessage(1410,
								l1iteminstance1.getName())); // 對\f1%0附加強大的魔法力量成功。
					...
					if (pc.getInventory().consumeItem(41246, 60)) {
						...
						|| (itemId == 47090) || (itemId == 47100)) { // 7階附魔石(近戰)(遠攻)(恢復)(防禦)
					if (pc.getInventory().consumeItem(41246, 100)) {
						...
						|| (itemId == 47091) || (itemId == 47101)) { // 8階附魔石(近戰)(遠攻)(恢復)(防禦)
					if (pc.getInventory().consumeItem(41246, 200)) {
						...
						|| (itemId == 47092) || (itemId == 47102)) { // 9階附魔石(近戰)(遠攻)(恢復)(防禦)
					...
					if (pc.isIllusionist() || pc.isGm()) {
						...
						|| (itemId == 40521)) { // 傳送回家的卷軸、象牙塔傳送回家的卷軸、精靈羽翼
					...
					if (pc.getMap().isUsePainwand()) {
						...
						if ((itemId == 40006) || (itemId == 140006)) {
							...
							if (l1iteminstance.getChargeCount() <= 0) { // 次數為
																		// 0時刪除
								pc.getInventory().removeItem(l1iteminstance, 1);
							}
						} else {
				...
				// TODO 自訂道具區
					...
					if (pc.isIllusionist() && (pc.getMapId() == 2004)) { // 異界
																			// 奎斯特
						boolean found = false;
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
					...
					if (pc.isDragonKnight() && (pc.getMapId() == 2004)) { // 異界
																			// 奎斯特
						boolean found = false;
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
				...
				if ((pc.isCrown() && l1iteminstance.getItem().isUseRoyal())
					...
					if ((min != 0) && (min > pc.getLevel())) {
						...
						pc.sendPackets(new S_PacketBox(
								S_PacketBox.MSG_LEVEL_OVER, max)); // 等級%d以下才能使用此道具。
					} else {
	...
	private void polyAction(L1PcInstance attacker, L1Character cha) {
		...
		if (cha instanceof L1PcInstance) {
			...
			if ((pc.getClanid() != 0)
					&& (attacker.getClanid() == pc.getClanid())) { // 目標為盟友
				isSameClan = true;
			}
		}
