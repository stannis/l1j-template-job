	...
	private void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				doll.setExpRateChance(rs.getInt("exp_rate_chance")); // 加倍機率
				doll.setExpRate(rs.getInt("exp_rate")); // 經驗值倍率
				doll.setIsHaste(rs.getByte("isHaste")); // 加速狀態
				doll.setAddHp(rs.getShort("hp")); // 增加HP
				doll.setAddMp(rs.getShort("mp")); // 增加MP
				doll.setHitModifier(rs.getShort("hit_modifier")); // 攻擊成功
				doll.setDmgModifier(rs.getShort("dmg_modifier")); // 額外攻擊點數
