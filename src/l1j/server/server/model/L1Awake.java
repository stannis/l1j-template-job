...
public class L1Awake {
	...
	public static void start(L1PcInstance pc, int skillId) {
		if (skillId == pc.getAwakeSkillId()) { // 再次咏唱時解除覺醒狀態
			...
			//doPoly(pc);
			doPoly(pc,skillId); // 變更 3.52C龍騎士覺醒狀態外型調整 by 0968026609
	...
	// 變身
	// 變更 3.52C龍騎士覺醒狀態外型調整 by 0968026609
	//public static void doPoly(L1PcInstance pc) {
	public static void doPoly(L1PcInstance pc, int skillId) {
		//int polyId = 6894;
		int polyId = pc.getClassId();
		if (skillId == AWAKEN_ANTHARAS) {//TODO 覺醒：安塔瑞斯
			polyId = 9362;
		} else if (skillId == AWAKEN_FAFURION) {//TODO 覺醒：法力昂
			polyId = 9364;
		} else if (skillId == AWAKEN_VALAKAS) {//TODO 覺醒：巴拉卡斯
			polyId = 9363;
		}
		// end
