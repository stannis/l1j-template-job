...
class L1ShopBuyOrder {
	...
	public L1ShopBuyOrder(L1ShopItem item, int count, int deleteDay, Timestamp deleteDate) { // 道具天數刪除系統
		_item = item;
		_count = count;
...
public class L1ShopBuyOrderList {
	...
	public void add(int orderNumber, int count) {
		...
		int deleteDay =  shopItem.getDeleteDay(); // 道具天數刪除系統(指定天數)
		Timestamp deleteDate =  shopItem.getDeleteDate(); // 道具天數刪除系統(指定日期)
		...
		if (shopItem.getItem().isStackable()) {
			_list.add(new L1ShopBuyOrder(shopItem, count * shopItem.getPackCount(), deleteDay, deleteDate)); // 道具天數刪除系統
			return;
		}

		for (int i = 0; i < (count * shopItem.getPackCount()); i++) {
			_list.add(new L1ShopBuyOrder(shopItem, 1, deleteDay, deleteDate)); // 道具天數刪除系統
		}
	}
