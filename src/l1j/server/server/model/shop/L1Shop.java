...
public class L1Shop {
	...
	private boolean ensureSell(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		if (_npcId >= Config.GASH_SHOP_MIN_ID
			...
			price = orderList.getTotalPrice(); // 無稅收
		} else {
		...
		if (_npcId >= Config.GASH_SHOP_MIN_ID
				&& _npcId <= Config.GASH_SHOP_MAX_ID) {
			howpay = Config.GASH_SHOP_ITEM_ID; // GASH幣
		} else {
			howpay = L1ItemId.ADENA; // 金幣
		}
		// end
		if (!pc.getInventory().checkItem(howpay, price)) { // GASH幣商城[提供:liumy]
			...
			if (howpay == L1ItemId.ADENA) {
				...
				pc.sendPackets(new S_ServerMessage(166, "商城專用貨幣不足"));
			}
			// end
			return false;
		}
	...
	private void sellItems(L1PcInstance pc, L1PcInventory inv, L1ShopBuyOrderList orderList) { // 增加 L1PcInstance pc,
		 ...
		 * IllegalStateException("不能消費為購買需要的金幣。"); }
		 */
		// GASH幣商城[提供:liumy]
		int howpay;
		if (_npcId >= Config.GASH_SHOP_MIN_ID
		...
		if (howpay == L1ItemId.ADENA) {
			...
			if (!inv.consumeItem(howpay, orderList.getTotalPrice())) {
				throw new IllegalStateException("不能消費為購買需要的商城專用貨幣。");
			}
		}
		// end
		for (L1ShopBuyOrder order : orderList.getList()) {
			...
			// 道具天數刪除系統
			int deleteDay = order.getDeleteDay(); // 道具天數刪除系統(指定天數)
			Timestamp deleteDate = order.getDeleteDate(); // 道具天數刪除系統(指定日期)
			// 道具天數刪除系統
			if (deleteDay > 0) { // ● 指定天數
