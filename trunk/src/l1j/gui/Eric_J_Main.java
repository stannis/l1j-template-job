...
public class Eric_J_Main extends javax.swing.JFrame implements ActionListener {
	...
	private void initComponents() {
		...
		SP_Split.setDividerLocation(700); //頻道顯示大小
	...
	private void submitMsg(int select) {
		...
		case 0:// 訊息頻
			L1World.getInstance().broadcastServerMessage(
					"線上GM：" + TF_Msg.getText());
			addWorldChat("線上GM", TF_Msg.getText());
			break;
		case 1:// 密頻
			...
			L1PcInstance target = L1World.getInstance().getPlayer(
					TF_Target.getText());
			target.sendPackets(new S_SystemMessage("線上GM(密語)："
					+ TF_Msg.getText()));
			addPrivateChat("線上GM(密語)", TF_Target.getText(), TF_Msg.getText());
			break;
		}
		TF_Msg.setText("");
	}
	...
	private void showItemTable(int num) {
		...
		case 1:// 1:倉庫
			...
			D_Item.setTitle(pc.getName() + "倉庫的物品(" + pc.getDwarfInventory().getSize() + ")");
		...
		case 2:// 2:血盟倉庫
			...
			D_Item.setTitle(pc.getClan().getClanName() + "血盟倉庫的物品(" + pc.getClan().getDwarfForClanInventory().getSize() + ")");
		...
		case 3:// 3:妖森倉庫
			...
			D_Item.setTitle(pc.getName() + "的妖森倉庫物品(" + pc.getDwarfForElfInventory().getSize() + ")");
	...
	private void setPlayerView(String name) {
		...
		F_Player.setTitle(pc.getName() + " 的基本資料");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("強制踢除(K)")) {
			...
			closeServer();
		} else if (command.equals("設定重開伺服器(E)")) {
			String temp = "";
			try {
		...
		} else if (command.equals("查詢重開時間(T)")) {
			int second = L1GameRestart.getInstance().GetRemnant();
			JOptionPane.showMessageDialog(this, "★ 距離伺服器重開時間還有 "
					+ (second / 60) / 60 + " 小時 " + (second / 60) % 60 + " 分 "
					+ second % 60 + " 秒 ★");
