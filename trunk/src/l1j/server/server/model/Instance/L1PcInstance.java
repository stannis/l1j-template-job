...
public class L1PcInstance extends L1Character {
	...
	public void startHasteByDoll() {
		if (getHasteItemEquipped() == 1) { // 判斷是否有裝備加速
	...
	public void stopHasteByDoll() {
		...
		if (getHasteItemByDoll() == 0) { // 判斷是否有裝備加速
			setMoveSpeed(0);
	...
	public void receiveDamage(L1Character attacker, double damage, boolean isMagicDamage) { // 攻撃でＨＰを減らすときはここを使用
		if ((getCurrentHp() > 0) && !isDead()) {
			...
			System.out.println("警告︰角色的ＨＰ減少處理有不正確的地方。※或者從最初開始ＨＰ０﹝L1PcInstance﹞");
			death(attacker);
		}
	}
	...
	public boolean isEinZone() { // 判斷是否在安全區域
		return getMap().isSafetyZone(getLocation());
	}

	public boolean isEinLevel() { // 判斷是否符合等級
		return _isEinLevel;
	}
	...
	private int _einPoint = 0; // 殷海薩的祝福 - %數
	private int _ein_getExp = 0; // 殷海薩的祝福 - 取得經驗點數的累計
	private static final int einMaxPercent = Config.EIN_MAX_PERCENT; // 殷海薩的祝福
