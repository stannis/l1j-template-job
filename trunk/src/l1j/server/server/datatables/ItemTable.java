...
public class ItemTable {
	...
	static {
		...
		_etcItemTypes.put("TeleportScroll", new Integer(18)); // 道具傳送卷軸(符)
		_etcItemTypes.put("ItemBlend", new Integer(19)); // 道具融合系統
		...
		// add 符石/遺物 : UseType= 43(右) 44(左-目前開放) 45(上) by srwh
	...
	private Map<Integer, L1EtcItem> allEtcItem() {
		...
		try {
			...
			while (rs.next()) {
				...
				item.setItemDescExtra(rs.getString("itemdesc_extra")); // 新增可從DB自行輸入鑑定描述
																		// by
																		// 13630020
				...
				item.setDelete_Day(rs.getInt("delete_day")); // 道具天數刪除系統(指定天數)
				item.setDelete_Date(rs.getTimestamp("delete_date")); // 道具天數刪除系統(指定日期)

				result.put(new Integer(item.getItemId()), item);
			}
		}
		catch (NullPointerException e) {
			_log.log(Level.SEVERE, new StringBuilder().append(item.getName()).append("(" + item.getItemId() + ")").append(" 載入失敗。").toString());
		}
	...
	private Map<Integer, L1Weapon> allWeapon() {
		...
		try {
			...
			while (rs.next()) {
				...
				weapon.setItemDescExtra(rs.getString("itemdesc_extra")); // 新增可從DB自行輸入鑑定描述
																			// by
																			// 13630020
			...
			_log.log(Level.SEVERE, new StringBuilder().append(weapon.getName()).append("(" + weapon.getItemId() + ")").append(" 載入失敗。")
					.toString());
		}
		catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
	...
	private Map<Integer, L1Armor> allArmor() {
		...
		try {
			...
			while (rs.next()) {
				...
				armor.setItemDescExtra(rs.getString("itemdesc_extra")); // 新增可從DB自行輸入鑑定描述
																		// by
																		// 13630020
				...
				armor.setDelete_Day(rs.getInt("delete_day")); // 道具天數刪除系統(指定天數)
				armor.setDelete_Date(rs.getTimestamp("delete_date")); // 道具天數刪除系統(指定日期)

				result.put(new Integer(armor.getItemId()), armor);
			}
		}
		catch (NullPointerException e) {
			_log.log(Level.SEVERE, new StringBuilder().append(armor.getName()).append("(" + armor.getItemId() + ")").append(" 載入失敗。")
					.toString());
		}
		catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
