...
public final class Config {
	...
	public static boolean GUI; // 管理介面開關
	...
	public static boolean GM_TALK; // ＧＭ使用公頻(&)顯示方式 1/5

	public static boolean WHO_ONLINE_MSG_ON; // 玩家上線是否通知在線ＧＭ 1/5

	public static boolean GM_Decrease_MP; // GM使用魔法是否扣魔 1/4

	public static int GASH_SHOP_MIN_ID; // GASH商城NPC編號最小值
	public static int GASH_SHOP_MAX_ID; // GASH商城NPC編號最大值
	public static int GASH_SHOP_ITEM_ID; // GASH商城專用貨幣的道具編號

	public static boolean FULL_HP_MP; // 玩家升級血魔是否補滿

	public static boolean NEW_CREATE_BROADCAST; // 創新角色是否公告

	public static boolean Attack_Mob_HP_Bar; // 攻擊顯示怪物血條

	public static short Hpr_InHotel; // [旅館]回血量
	public static short Mpr_InHotel; // [旅館]回魔量

	public static short Hpr_InHouse; // [血盟小屋]回血量
	public static short Mpr_InHouse; // [血盟小屋]回魔量

	public static short Hpr_InCastle; // [城堡內城]回血量
	public static short Mpr_InCastle; // [城堡內城]回魔量

	public static short Hpr_UnderTheTree; // [妖森大樹下]回血量
	public static short Mpr_UnderTheTree; // [妖森大樹下]回魔量

	public static byte PET_MAX_LV; // 設定寵物最高等級

	public static double RATE_XP_PET; // 設定寵物經驗值倍率

	public static boolean AUTO_ADD_SKILL; // 設定是否開啟自動學習技能

	public static boolean LEVEL_UP_REWARD; // 設定是否開啟升級獎勵道具系統

	public static boolean MOVE_SEND_BONUSSTATS; // 設定是否開啟免登出提示升級能力值點數
	...
	public static boolean LEVEL_BONUS; // 設定是否開啟經驗值回饋獎勵系統

	public static boolean LOGIN_GAME_DELAY; // 設定登入遊戲是否給予延遲

	public static boolean NEW_CREATE_SET_GM; // 設定創新角色是否設定為GM

	public static boolean OL_ENQUIRE_RATES; // 是否開放線上查詢伺服器配置資訊

	public static boolean WHO_ONLINE_LIST; // 是否可以查詢線上所有名單 1/3
	
	public static boolean PP_Is_Active; // 設定是否使用指令/誰，顯示幽靈人口機制
	public static short Phantom_Population; // 幽靈人口固定增加人數
	public static short Phantom_Population_Random; // 幽靈人口浮動人數
	
	/** 公頻&買賣頻道發話延遲 **/
	public static short Chat_Delay_Time;

	/** EinhasadSettings control */
	public static boolean EINHASAD_IS_ACTIVE; // 是否啟動殷海薩的祝福系統

	public static short EIN_TIME;// 登入多久時間取得1%

	public static short EIN_OUTTIME;// 登出多久時間取得1%
	...
	public static final String FAME_SETTINGS_CONFIG_FILE = "./config/famesystem.properties"; // 聲望系統
	...
	public static void load() {
		...
		try {
			...
			GM_TALK = Boolean.parseBoolean(otherSettings.getProperty(
					"GMSpeakName", "false")); // ＧＭ使用公頻(&)顯示方式 2/5
			WHO_ONLINE_MSG_ON = Boolean.parseBoolean(otherSettings.getProperty(
					"WhoOnlineMessageOn", "false")); // 玩家上線是否通知在線ＧＭ 2/5
		...
		try {
			...
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
			throw new Error("無法讀取設定檔: " + FAME_SETTINGS_CONFIG_FILE);
		}
	...
	public static boolean setParameterValue(String pName, String pValue) {
		...
		/** 連線線程開關 */
			...
			GUI = Boolean.parseBoolean(pValue); // 管理介面開關
		}
		// rates.properties
		...
		// othersettings.properties
			...
			GM_TALK = Boolean.valueOf(pValue); // ＧＭ使用公頻(&)顯示方式 3/5
		} else if (pName.equalsIgnoreCase("WhoOnlineMessageOn")) {
			WHO_ONLINE_MSG_ON = Boolean.valueOf(pValue); // 玩家上線是否通知在線ＧＭ 3/5
			...
			GASH_SHOP_MIN_ID = Integer.valueOf(pValue); // GASH商城NPC編號最小值
		} else if (pName.equalsIgnoreCase("GSshopMaxID")) {
			GASH_SHOP_MAX_ID = Integer.valueOf(pValue); // GASH商城NPC編號最大值
		} else if (pName.equalsIgnoreCase("GSItemID")) {
			GASH_SHOP_ITEM_ID = Integer.valueOf(pValue); // GASH商城專用貨幣的道具編號	
		} else if (pName.equalsIgnoreCase("FullHPMP")) {
			FULL_HP_MP = Boolean.valueOf(pValue); // 玩家升級血魔是否補滿
		} else if (pName.equalsIgnoreCase("NewCreateBroadcast")) {
			NEW_CREATE_BROADCAST = Boolean.valueOf(pValue); // 創新角色是否公告
		} else if (pName.equalsIgnoreCase("AttackMobHPBar")) {
			Attack_Mob_HP_Bar = Boolean.valueOf(pValue); // 攻擊顯示怪物血條
		} else if (pName.equalsIgnoreCase("HprInHotel")) {
			Hpr_InHotel = Short.valueOf(pValue); // [旅館]回血量
		} else if (pName.equalsIgnoreCase("MprInHotel")) {
			Mpr_InHotel = Short.valueOf(pValue); // [旅館]回魔量
			...
			Hpr_InCastle = Short.valueOf(pValue); // [城堡內城]回血量
		} else if (pName.equalsIgnoreCase("MprInCastle")) {
			Mpr_InCastle = Short.valueOf(pValue); // [城堡內城]回魔量
		} else if (pName.equalsIgnoreCase("HprUnderTheTree")) {
			Hpr_UnderTheTree = Short.valueOf(pValue); // [妖森大樹下]回血量
		} else if (pName.equalsIgnoreCase("MprUnderTheTree")) {
			Mpr_UnderTheTree = Short.valueOf(pValue); // [妖森大樹下]回魔量
		} else if (pName.equalsIgnoreCase("PetMaxLV")) {
			PET_MAX_LV = Byte.valueOf(pValue); // 設定寵物最高等級
		} else if (pName.equalsIgnoreCase("PetRateXp")) {
			RATE_XP_PET = Double.valueOf(pValue); // 設定寵物經驗值倍率
		} else if (pName.equalsIgnoreCase("AutoAddSkill")) {
			AUTO_ADD_SKILL = Boolean.valueOf(pValue); // 設定是否開啟自動學習技能
		} else if (pName.equalsIgnoreCase("LevelUpReward")) {
			LEVEL_UP_REWARD = Boolean.valueOf(pValue); // 設定是否開啟升級獎勵道具系統
		} else if (pName.equalsIgnoreCase("MoveSendBonusStats")) {
			MOVE_SEND_BONUSSTATS = Boolean.valueOf(pValue); // 設定是否開啟免登出提示升級能力值點數
			...
			LEVEL_BONUS = Boolean.valueOf(pValue); // 設定是否開啟經驗值回饋獎勵系統
		} else if (pName.equalsIgnoreCase("LoginGameDelay")) {
			LOGIN_GAME_DELAY = Boolean.valueOf(pValue); // 設定登入遊戲是否給予延遲
		} else if (pName.equalsIgnoreCase("NewCreateSetGM")) {
			NEW_CREATE_SET_GM = Boolean.valueOf(pValue); // 設定創新角色是否設定為GM
		} else if (pName.equalsIgnoreCase("OnlineEnquireRates")) {
			OL_ENQUIRE_RATES = Boolean.valueOf(pValue); // 是否開放線上查詢伺服器配置資訊
		} else if (pName.equalsIgnoreCase("WhoOnlineList")) {
			WHO_ONLINE_LIST = Boolean.valueOf(pValue); // 是否可以查詢線上所有名單 3/3
		} else if (pName.equalsIgnoreCase("PPIsActive")) {
			PP_Is_Active = Boolean.valueOf(pValue); // 設定是否使用指令/誰，顯示幽靈人口機制
		} else if (pName.equalsIgnoreCase("PhantomPopulation")) {
			Phantom_Population = Short.valueOf(pValue); // 幽靈人口固定增加人數
		} else if (pName.equalsIgnoreCase("PhantomPopulationRandom")) {
			Phantom_Population_Random = Short.valueOf(pValue); //  幽靈人口浮動人數
